...
namespace EGO.Framework
{
    public class TreeNode : VerticalLayout
    {
        ...
        public TreeNode(bool spread, string content,int indent = 0)
        {
            ...
            Style = new FluentGUIStyle(()=>new GUIStyle(EditorStyles.foldout));
            
            mFirstLine.AddTo(this);
            mFirstLine.AddChild(new SpaceView(indent));

            new CustomView(() => { Spread.Value = EditorGUILayout.Foldout(Spread.Value, Content, true,Style.Value); }).AddTo(mFirstLine);
