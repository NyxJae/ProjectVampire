...
namespace QFramework.Pro
{
    ...
    public class ArchitectureGraph : GUIGraph
    {
        ...
        public List<SystemNode> Systems => nodes.Where(n => n is SystemNode).Cast<SystemNode>().ToList();

        public List<ModelNode> Models => nodes.Where(n => n is ModelNode).Cast<ModelNode>().ToList();

        public List<BindablePropertyNode> BindableProperties =>
            nodes.Where(n => n is BindablePropertyNode).Cast<BindablePropertyNode>().ToList();
        
        public List<SimpleClassNode> SimpleClassNodes => nodes.Where(n => n is SimpleClassNode).Cast<SimpleClassNode>().ToList();
    ...
    public class ArchitectureGraphInspector : GUIGlobalGraphInspector
    {
        ...
        public override void OnInspectorGUI()
        {
                ...
                if (GUILayout.Button("..."))
                {
                    ...
                    else
                    {
                        if (folderPath.EndsWith(mProjectName.stringValue))
                        {
                            ...
                            mGenerateFolder.stringValue =
                                folderPath.Remove(folderPath.Length - ("/" + mProjectName.stringValue).Length);
                        }
                        else
                        {
            ...
            GUILayout.Label(mArchitectureGraph.ScriptsFolderPath + "/System");
    ...
    public class ArchitectureGraphEditor : GUIGraphEditor
    {
